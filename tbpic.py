...
def tbpicdl(id):
	...
	try:
		try:
			s=soup.find_all('dl',attrs={'class':'tb-prop tm-sale-prop tm-clear tm-img-prop '})[0]#列表内元素，链接多商品的情况下取第一个商品图片链接
		except:
			s=soup.find_all('img',attrs={'id':'J_ImgBooth'})[0]#解决颜色等分类描述没有图片的情况，按照第一张预览图取主图
		finally:
		...
		try:
			pattern=re.compile('//(.*?)_\d+x\d+q90.jpg')
			result=pattern.findall(str(soup))[0]#直接从网页源获取
			picurl='https://'+result+'_800x800q90.jpg'
			urllib.request.urlretrieve(picurl, "%s.jpg" % str(id))
		except:
